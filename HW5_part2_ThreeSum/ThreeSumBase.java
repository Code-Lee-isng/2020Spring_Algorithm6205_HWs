...
/**
 * File Name: ThreeSumBase.java
 * <p>
 * To Compile: IntUtil.java RandomInt.java ThreeSumBase.java
 *
...
abstract class ThreeSumBase {
    //input
    protected int[] a;
    protected int n;
    protected static boolean show = true;
    //required
    protected IntUtil u = new IntUtil();

    //I don't know how to write it
    //Override by the concrete class
    protected abstract List<List<Integer>> threeSum(int[] nums, int n, int method);

    protected abstract String inputFileBase();

    protected void testBench() {
        simpleTests();
        System.out.println("All test cases passed. You will get full grade now");
        System.out.println("Include only ThreeSum.java and the output of this program for FULL grade");
    }
    ...
    private int[] readFile(String s) {
        ...
        while (scanner.hasNextLong()) {
            //-1988258329 -1862536728 -444172239
            Long v = scanner.nextLong();
            if (v < Integer.MIN_VALUE) {
                System.out.println("Will not read" + v);
                continue;
            }
    ...
    /*
     * Print all 3 integers that adds to n
     * O(n)
     */
    private void P(List<List<Integer>> ans) {
        if (show) {
            if (ans.size() != 0) {
    ...
    public int compare3(List<Integer> x, List<Integer> o) {
        int s1 = x.size();
        int s2 = o.size();
        u.myassert(s1 == s2);
        u.myassert(s1 == 3);
        ...
        Collections.sort(x);
        Collections.sort(o);
        int a = compare2(x.get(0), o.get(0));
        if (a == 0) a = compare2(x.get(1), o.get(1));
        if (a == 0) a = compare2(x.get(2), o.get(2));
        return a;
    }

    /*
     * This will sort ArrayList of ArrayLists of Integer[2]
     */
    private class ListOfListComparator implements Comparator<List<Integer>> {
        private boolean ascend;

        ListOfListComparator(boolean ascend) {
            this.ascend = ascend;
        }
    ...
    private void ListOfListSort(List<List<Integer>> p, boolean ascend) {
        ListOfListComparator e = new ListOfListComparator(ascend);
        Collections.sort(p, e);
    }

    /*
     * make sure every algorithm produces same list
     */
    private void assert2ListsSame(List<List<Integer>> ans, List<List<Integer>> ans2) {
        /* make sure every algorithm produces same list */
        int as = ans.size();
        int as2 = ans2.size();
        u.myassert(as == as2);
        ListOfListSort(ans, true);
        ListOfListSort(ans2, true);

        for (int i = 0; i < as; ++i) {
            List<Integer> alist = ans.get(i);
            List<Integer> blist = ans2.get(i);
            int s = alist.size();
            ...
            if (failed) {
                ...
                P(ans2);
                for (int j = 0; j < s; ++j) {
                    int t0 = alist.get(0) + alist.get(1) + alist.get(2);
                    System.out.println(alist.get(0) + " " + alist.get(1) + " " + alist.get(2) + " = " + t0);
                    int t1 = blist.get(0) + blist.get(1) + blist.get(2);
                    System.out.println(blist.get(0) + " " + blist.get(1) + " " + blist.get(2) + " = " + t1);
                    u.myassert(false);
                }
            }
        }
    ...
    /*
     * copy gold array to a, as algorithm can modify a
     */

    private List<List<Integer>> run1(int x, String s, final int[] gold) {
        a = new int[gold.length];
        for (int i = 0; i < gold.length; ++i) {
            a[i] = gold[i];
        }
        long startTime = System.nanoTime();
        List<List<Integer>> ans = threeSum(a, n, x);
        long endTime = System.nanoTime();
        double d = u.timeInSec(endTime, startTime);
        System.out.println(s + "  n = " + a.length + " CPU = " + d + " secs" + " Num Tuples = " + ans.size());
        return ans;
    }

    private void copy(final int[] gold) {
        a = new int[gold.length];
        for (int i = 0; i < gold.length; ++i) {
            a[i] = gold[i];
        }
     ...
     * 3 = run only 3
     */
    private void test1(int x, int e) {
        //Keep a gold copy as algorithms can change nums
        int[] gold = new int[a.length];
        for (int i = 0; i < gold.length; ++i) {
            gold[i] = a[i];
        }
        System.out.println("---------------------------------------------");
        System.out.println("Array size = " + a.length + " Looking for n = " + n);
        System.out.println("---------------------------------------------");
        if (show) {
            if (a.length < 20) {
                u.pLn(a.length);
        ...
        List<List<Integer>> ans1 = null;
        List<List<Integer>> ans2 = null;
        List<List<Integer>> ans3 = null;
        if (x == 123) {
            ans1 = run1(1, "O(n^3)Time O(1)Space Alg", gold);
            ans2 = run1(2, "O(n^2)Time O(n)Space Alg", gold);
            ans3 = run1(3, "O(n^2)Time O(1)Space Alg", gold);
        }
        if (x == 23) {
            ans2 = run1(2, "O(n^2)Time O(n)Space Alg", gold);
            ans3 = run1(3, "O(n^2)Time O(1)Space Alg", gold);
        ...
        if (ans1 != null && ans2 != null && ans3 != null) {
            if ((ans1.size() != e) || (ans2.size() != e)) {
                System.out.println("-------ans from 1 ------------");
                P(ans1);
                System.out.println("-------WRONG answer from 2------------");
                P(ans2);
                u.myassert(false);
                ;
            }
            if ((ans1.size() != e) && (ans3.size() != e)) {
                System.out.println("-------ans from 1 ------------");
                P(ans1);
                System.out.println("-------WRONG answer from 3 ------------");
                P(ans3);
                u.myassert(false);
            }
            assert2ListsSame(ans1, ans2);
            assert2ListsSame(ans2, ans3);
            P(ans1);
        }

        if (ans1 == null && ans2 != null && ans3 != null) {
            if ((ans2.size() != e) || (ans3.size() != e)) {
                System.out.println("-------ans from 2 ------------");
                P(ans2);
                System.out.println("-------WRONG answer from 3------------");
                P(ans3);
                u.myassert(false);
    ...
    private void simpleTests() {
        ...
        {
            int b[] = {-4, -3, -3, -3, -3, 1, 1, 1, 2, 2, 3, 3, 3, 6};
            int n = 0;
            this.a = b;
            this.n = n;
            test1(123, 4);
        ...
        {
            int b[] = {-25069, -9243, 34312, -55692, -9243, 64935};
            int n = 0;
            this.a = b;
            this.n = n;
            test1(123, 2);
    ...
    public static void main(String[] args) {
        System.out.println("ThreeSumBase.java STARTS");
        String version = System.getProperty("java.version");
        System.out.println("Java version used for this program is " + version);
        System.out.println("You cannot instantiate ThreeSumBase class: ThreeSumBase p = new ThreeSumBase() ; ");
        //ThreeSumBase p = new ThreeSumBase() ;
        System.out.println("ThreeSumBase.java ENDS");
    }
}